DROP TABLE IF EXISTS amea.temp_DRM_IMS_Delta;
CREATE TABLE amea.temp_DRM_IMS_Delta AS
SELECT CASE
...
HAVING CASE
WHEN LENGTH(FCST.yearperiod)=7 then CAST(CONCAT(SUBSTR(FCST.yearperiod,-4),SUBSTR(FCST.yearperiod,2,2)) AS INT) ELSE CAST(CONCAT(SUBSTR(FCST.yearperiod,-4),SUBSTR(FCST.yearperiod,2,1)) AS INT) END >= CASE WHEN LENGTH(CONCAT(FCST.YEAR,FCST.PERIOD))= 6 then CAST(CONCAT(FCST.YEAR,'0',FCST.PERIOD) AS INT) ELSE CAST(CONCAT(FCST.YEAR,FCST.PERIOD) AS INT) END
) AS ACTUAL
INNER JOIN
(
SELECT FCST.distributorcode DISTRIBUTORCODE
...
on (DFUMap.new_dfu=FCST.dfucode)
WHERE FCST.period between (CASE WHEN ${hivevar:period}-5 < 1 THEN 0 ELSE 1 END) and (CASE WHEN ${hivevar:period}-5 < 1 THEN 0 ELSE ${hivevar:period}-2 END) --reporting period parameter
AND FCST.year = ${hivevar:year} --reporting year parameter
GROUP BY FCST.distributorcode
,DFUMap.BRAND
,DFUMap.new_dfu
...
HAVING
CASE
WHEN LENGTH(FCST.yearperiod)=6 then SUBSTR(FCST.yearperiod,2,1) ELSE SUBSTR(FCST.yearperiod,2,2) END = CAST(fcst.period AS INT)
...
JOIN amea.src_ext_txt_menat_pathways_dfu_mapping DFUMap
on (DFUMap.new_dfu=FCST.dfucode)
WHERE FCST.period between (CASE WHEN ${hivevar:period}-5 < 1 THEN 1 ELSE 0 END) and (CASE WHEN ${hivevar:period}-5 < 1 THEN ${hivevar:period}-2 ELSE 0 END) --reporting period parameter
AND FCST.year = ${hivevar:year} --reporting year parameter
GROUP BY FCST.distributorcode
,DFUMap.BRAND
,DFUMap.new_dfu
...
JOIN amea.src_ext_txt_menat_pathways_dfu_mapping DFUMap
on (DFUMap.new_dfu=FCST.dfucode)
WHERE FCST.period between (CASE WHEN ${hivevar:period}-5 < 1 THEN ${hivevar:period}+8 ELSE 0 END) and (CASE WHEN ${hivevar:period}-5 < 1 THEN 12 ELSE 0 END) --reporting period parameter
