...
select 
  ...
  upper(material.category) as category,
   ...
   upper(customer.country_region) as country_region,
   upper(customer.bu_region) as bu_region
from amea.pcd_ext_orc_menat_ims_material material   -- to be change to pcd_ext_orc_menat_ims_actual_daily when table is working
   inner join
   (select daily.year, daily.period, daily.distributorcode, daily.kelloggskucode, max(daily.salesincases) salesincases
from amea.archive_ext_txt_menat_ims_actual_daily daily
where daily.load_date <= add_months(current_date,-1)
group by daily.year, daily.period, daily.distributorcode, daily.kelloggskucode
  ) ims_daily_lmtd
